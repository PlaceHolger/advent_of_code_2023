...
void PrintMap(int highlightX = -1, int highlightY = -1)
{
    for (int y = 0; y < NUM_ROWS; ++y)
    {
        for (int x = 0; x < NUM_COLUMNS; ++x)
        {
            ...
            else if(isInClosed && isInOpen)
                std::cout << "\033[1;34m " << s_Data[y][x];
            else if (isInClosed)
...
void CalculatePath(int startX, int startY, int targetX, int targetY)
{
    ...
    while (!openList.empty())
    {
        ...
        const Direction lastDirEnum = p_CurrentNode->lastDirections[2];
        const int lastDir = static_cast<int>(lastDirEnum);

        int numDirPlus1 = std::min(MAX_SAME_DIRECTION - 1, numSameDirection+1); //clamped, will be removed anyway in the next step.
        //get the current node's neighbours
        Node* p_Neighbours[4] = { nullptr, nullptr, nullptr, nullptr };
        if (p_CurrentNode->posX > 0) //left
            p_Neighbours[0] = &nodes[(lastDirEnum == Direction::Left) ? numDirPlus1 : 0][lastDir][p_CurrentNode->posY * NUM_COLUMNS + (p_CurrentNode->posX - 1)];
        if (p_CurrentNode->posX < NUM_COLUMNS - 1) //right
            p_Neighbours[1] = &nodes[(lastDirEnum == Direction::Right) ? numDirPlus1 : 0][lastDir][p_CurrentNode->posY * NUM_COLUMNS + (p_CurrentNode->posX + 1)];
        if (p_CurrentNode->posY > 0) //up
            p_Neighbours[2] = &nodes[(lastDirEnum == Direction::Up) ? numDirPlus1 : 0][lastDir][(p_CurrentNode->posY - 1) * NUM_COLUMNS + p_CurrentNode->posX];
        if (p_CurrentNode->posY < NUM_ROWS - 1) //down
            p_Neighbours[3] = &nodes[(lastDirEnum == Direction::Down) ? numDirPlus1 : 0][lastDir][(p_CurrentNode->posY + 1) * NUM_COLUMNS + p_CurrentNode->posX];
        ...
        {            
            ...
            if (cost < p_Neighbours[i]->cost)
            {
                ...
                p_Neighbours[i]->lastDirections[2] = static_cast<Direction>(i); //the direction we are going to take to get to the neighbour
