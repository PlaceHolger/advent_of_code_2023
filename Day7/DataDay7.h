...
constexpr int NUM_CARDS = 5;
constexpr int NUM_DIFFERENT_CARDS = std::size("AKQJT98765432");
...
struct Hand
{
    Hand(const char cards[NUM_CARDS+1], int bid) : m_Bid(bid), m_Score(0)  //+1 for the null terminator, just makes parsing easier
    {
        ...
        else if(s_MaxEqualAmount == 4) //four of a kind
        {
            m_Score |= 5 << 24;
        }
        else if(s_MaxEqualAmount == 3 && s_SecondMaxEqualAmount == 2) //full house
        {
            m_Score |= 4 << 24;
        }
        else if(s_MaxEqualAmount == 3) //three of a kind
        {
            m_Score |= 3 << 24;
        }
        else if(s_MaxEqualAmount == 2 && s_SecondMaxEqualAmount == 2) //two pairs
        {
            m_Score |= 2 << 24;
        }
        else if(s_MaxEqualAmount == 2) //one pair
        {
            m_Score |= 1 << 24;
        }
    }	
...
private:
    void UpdateEqualAmount(const char cards[NUM_CARDS+1]) const
    {
        int charCount[NUM_DIFFERENT_CARDS] = {0};

        for (int i = 0; i < NUM_CARDS; i++)
        {
            charCount[GetCardValue(cards[i])]++;
        }
        ...
        for(int i = 1; i < NUM_DIFFERENT_CARDS; ++i)
        {
            ...
            if (count >= s_MaxEqualAmount)
            {
                ...
                s_MaxEqualAmount = count;
            }
            else if (count > s_SecondMaxEqualAmount)
            {
